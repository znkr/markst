...
// VisitChildren calls fn(x) on each of n's non-nil child expressions x. If any
// call returns true, VisitChildren stops and returns true. Otherwise,
// VisitChildren returns false.
//
// Note that VisitChildren(n, fn) only calls fn(x) for n's immediate children.
// If x's children should be processed, then fn(x) must call VisitChildren(x,
// fn).
//
// VisitChildren allows constructing general traversals of the IR graph that can
// stop early if needed. The most general usage is:
//
...
//	fn(root)
//
// Since VisitChildren does not return true itself, if the fn function never
// wants to stop the traversal, it can assume that VisitChildren itself will
// always return false, simplifying to:
//
//	var fn func(ir.Expr) bool
//	fn = func(x ir.Expr) bool {
//	    ... processing BEFORE visiting children ...
...
// Visit visits each expression x in the IR tree rooted at n in a depth-first
// preorder traversal, calling visit on each node visited.
...
// Any looks for an expression x in the IR tree rooted at n for which cond(x)
// returns true. Any considers nodes in a depth-first, preorder traversal.
//
// When Any finds a node x such that cond(x) is true, Any ends the traversal and
// returns true immediately. Otherwise Any returns false after completing the
// entire traversal.
...
// visitDestructPatterns visits the expressions contained in destructuring
// patterns.
